...
## Simple example: Regress Y on X and X^2   
```{r slr}
nobs <- 20
...
Why are estimates so different?  
  ...
  * detail: mean of X^2 is not (mean X)^2, so should center variables ourselves (same principle for interactions)

Redo with hand-centered variables  
```{r slr2}
...
Very useful if you want a transformation of parameters, e.g. X where quadratic is min or max
```{r max}
